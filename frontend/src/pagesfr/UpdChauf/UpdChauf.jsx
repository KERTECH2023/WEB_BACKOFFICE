...
const UpdChauf = () => {
  ...
  const fetchData = async () => {
    try {
      const [chauffeurRes, vehicleRes] = await Promise.all([
        axios.get(`${process.env.REACT_APP_BASE_URL}/Chaufffr/searchchauf/${id}`),
        axios.get(`${process.env.REACT_APP_BASE_URL}/Voifr/getvoi/${id}`)
      ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await Promise.all([
        axios.put(
          `${process.env.REACT_APP_BASE_URL}/Chaufffr/updatechauf/${id}`,
          chauffeurFormData,
          { headers: { "Content-Type": "multipart/form-data" } }
        ),
        axios.put(
          `${process.env.REACT_APP_BASE_URL}/Voifr/updatevoi/${formData.vehicle.id}`,
          vehicleFormData,
          { headers: { "Content-Type": "multipart/form-data" } }
        )
      ]);
