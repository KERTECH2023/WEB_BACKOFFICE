...
import List from "../../componentsfr/table/Table";
import ListVoi from "../../componentsfr/tablevoiture/TableVoi";
...
const SingleC = () => {
  ...
  const getSingleUser = async (id) => {
    const response = await axios.get(
      process.env.REACT_APP_BASE_URL + `/Chaufffr/searchchauf/${id}`
    );
  ...
  const handleSubmit = () => {
    axios
      .put(process.env.REACT_APP_BASE_URL + `/Chaufffr/updatestatus/${id}`, {
        headers: {
  ...
  const handleSendRejection = async () => {
    ...
    try {
      await axios.post(process.env.REACT_APP_BASE_URL + `/chaufffr/${id}/reject`, { reason: rejectionReason });
