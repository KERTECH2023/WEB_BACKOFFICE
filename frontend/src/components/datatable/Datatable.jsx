...
const Datatable = () => {
  ...
  const getUsers = async () =>{
    const response = await axios.get(process.env.NEXT_PUBLIC_BASE_URL + "/agent/getAg");
  ...
  //   ){
  //     const response = await axios.delete(`process.env.NEXT_PUBLIC_BASE_URL + /agent/destroyAg/${id}`);
  //     if(response.status===200){
      ...
      renderCell: (params) => {
        return (
          <div className="cellAction">
            <Link to={`/cosnult/${params.row.id}`} style={{ textDecoration: "none" }}>
